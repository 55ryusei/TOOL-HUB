...
export function getAllTools(): Tool[] {
  const defaults: Tool[] = defaultTools.map((t) => ({ ...t, source: 'default' as const }))
  const users: Tool[] = loadUserTools().map((t) => ({ ...t, source: 'user' as const }))
...
/** ユーザーツールを削除（デフォルトは消せない）。保存後の全リストを返す。 */
export function removeTool(id: string): Tool[] {
  const tools = loadUserTools().filter((t) => t.id !== id)
  saveUserTools(tools)
  // order からも除去しておく
